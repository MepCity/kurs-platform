...
| `platform_administrators`, `platform_administrator_profiles` | `SELECT` | `FORCE RLS`; yalnız `IAM_AUTH` scope'unda `PLATFORM_ADMIN_ACTIVATE` için aktörün kendi admin kaydı doğrulaması (bkz. aşağıdaki satır). `platform_administrator_profiles` yalnız `IAM_AUTH` `PLATFORM_ADMIN_ACTIVATE` kapsamında açılır. ORGANIZATION ve GLOBAL scope platform_administrators SELECT policy'leri `iam_runtime`'a ait değildir; bu policy davranışları `ORG-001` §4.1a–§4.1b'de tanımlıdır, somut runtime rolü ve `TO <role>`/`GRANT` ifadeleri `ORG-003`'te kesinleşecektir. |
