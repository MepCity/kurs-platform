...
İlk sürümün ana işlem veritabanı standart PostgreSQL'dir. Yönetilen production adayı
**Supabase PostgreSQL**, tercih edilen bölge **AWS `eu-central-1` (Frankfurt)** olarak korunur;
ancak kaynaklar kullanım kademesine göre açılır. Yerel geliştirme PostgreSQL'i yerelde çalışır.
Yalnız sentetik veri kullanan kapalı alfa Supabase Free veya öğrenci kredili eşdeğer yönetilen
PostgreSQL kullanabilir. Gerçek kurum pilotunda otomatik duraklamayan, yedekli ücretli katmana
geçilir. Small compute + PITR ilk gün zorunluluğu değildir; kabul edilmiş RPO/RTO veya ölçülmüş
veri kaybı riski oluştuğunda etkinleştirilir.
...
İlgili provisioning/uygulama görevi READY olmadan hiçbir sağlayıcı hesabı, üretim ortamı, sır
veya migration oluşturulmaz.
...
| AB veri yerleşimi ve yakın bölge | İlk kullanıcıların Türkiye'de olması için gecikmeyi azaltır; Frankfurt tercih edilir, fakat kesin Frankfurt ancak hukukî/gecikme gereksinimiyle zorunlu olur. |
...
| Maliyet görünürlüğü | Kapalı alfa, gerçek kurum pilotu ve kritik production maliyetleri ayrı gösterilmelidir; ücretsiz katman gerçek öğrenci verisinin güvenilir production'ı sayılmaz. |
...
|---|---|---:|---|
| Supabase | Pro plan 25 USD + Small compute 15 USD + 7 gün PITR yaklaşık 100 USD − 10 USD compute kredisi | **yaklaşık 130** | PITR en az Small compute gerektirir; 2 dakikaya kadar RPO belgesi vardır ve kritik-production karşılaştırma tabanıdır. |
...
Bu yaklaşık `130 USD/ay` satırı başlangıç production bütçesi değil, düşük RPO isteyen kritik
production profilidir. Kademeli başlangıç şöyledir:

| Kademe | PostgreSQL çalışma biçimi | Maliyet amacı |
|---|---|---:|
| Yerel geliştirme | Yerel PostgreSQL/Testcontainers, yalnız sentetik veri | `0 USD/ay` |
| Kapalı alfa | Supabase Free veya öğrenci kredili yönetilen PostgreSQL; SLA yok, duraklama kabul | dış ödeme hedefi `0 USD/ay` |
| Gerçek kurum pilotu | Duraklamayan ücretli PostgreSQL, otomatik günlük yedek ve bağımsız şifreli dump | toplam platform bütçesi içinde `25–60 USD/ay` |
| Aktif/kritik ürün | Ölçüme göre compute, sürekli staging ve kabul edilmiş RPO/RTO varsa PITR | ihtiyaç ve yazılı onaya göre |

Ücretsiz katman yalnız bütün ürün verisi sentetik olan kapalı alfa içindir. Davetli test
kullanıcıları gerçek kişiler olabilir; kullanıcı adları tercihen takma olur ve kurum, öğrenci,
veli, yoklama, ilerleme, PDF ve rapor verileri gerçek kişiyi/kurumu temsil etmez. Herhangi bir
gerçek kurum veya öğrenci verisi kullanıcı sayısından bağımsız ücretli pilot profiline geçiştir.

## 5. Önerilen teknik sınırlar

### 5.1. Ortamlar ve bölge

- `development`, `staging` ve `production` mantıksal olarak ayrıdır; ilk günden üç kalıcı bulut
  projesi açılması gerekmez. Development yerel, staging yayın öncesi geçici olabilir.
- Gerçek production ve gerektiğinde staging AB içinde, tercihen `eu-central-1` (Frankfurt)
  bölgesinde oluşturulur. Üretim kişisel verisi development/staging'e kopyalanmaz.
- Düzenli gerçek kurum kullanımı ücretsiz/otomatik askıya alınabilen katmanda çalıştırılmaz.
...
- Migration sahibi runtime rolünden ayrıdır. Migration aracı/yürütme modeli **ayrı bir teknik karar** olarak modül migration'ları başlamadan önce kapanmalıdır; bu ADR veya A-002 bu aracı seçmez. Repo/CI iskeleti yalnız onaylı aracı bağlar; her modül görevi yalnız kendi yeni migration dosyasının sahibidir. Migration sahibi, yalnız CI/CD'nin kısa ömürlü sırrıyla kullanılır ve production çalışma zamanında kullanılmaz.
...
- `users`, `user_identities`, `platform_administrators`, `platform_administrator_profiles` ve `trusted_devices` globaldir; `context_selection_tokens`, `refresh_token_families`/`refresh_tokens` kullanıcı-cihaz sahibine bağlı **karma** tablolardır ve kurum ailesinde `organization_membership_id` taşır (`VERI_MODELI.md` §4.1–§4.11a). Bunlar “kurum eşitse” genel policy'siyle görünür kılınamaz. Varsayılanı kapalı erişimdir: `app_runtime` rolüne doğrudan tablo/sequence hakkı verilmez. Sağlayıcıdan bağımsız ayrı `iam_runtime` rolü yalnız IAM tabloları ile audit/idempotency/provider-command outbox yüzeyine en az yetkiyle erişir; owner, superuser ve `BYPASSRLS` olamaz. Kurum kapsamlı IAM işlemleri `FORCE RLS` + transaction içi `SET LOCAL` bağlamıyla sınırlandırılır.
- `iam_runtime` transaction'ı `app.iam_operation_scope` ile tam olarak `ORGANIZATION`, `GLOBAL`, `AUTHENTICATION` veya allow-listli `IAM_PROVISIONING` seçer. İlk ikisi doğrulanmış actor ister; `AUTHENTICATION` yalnız server-set issuer+subject ile tam identity lookup yapar; provisioning yalnız actor+kurum+önceden üretilmiş target+operation code ile gereken yaratma yüzeyini açar. Bu dar kapsamlar genel GLOBAL+ORGANIZATION birleşimi değildir. Aynı transaction'da scope değişirse önceki `SET LOCAL` değerleri temizlenir; iki kapsamın değerleri birlikte bulunursa tüm ilgili `USING`/`WITH CHECK` policy'leri reddeder. Karma refresh tablolarında kurum policy'si `SELECT`/`INSERT`/`UPDATE`, global hedef-kullanıcı policy'si yalnız `SELECT`/`UPDATE` verir; birleşik role grant global bağlamda `INSERT` yetkisi doğurmaz. `audit_logs`, `idempotency_keys` ve `iam_provider_commands` da `FORCE RLS` altındadır; kapsam eşleşmezse varsayılan red uygulanır.
...
- Gerçek kurum pilotunda en az otomatik günlük yedek ve bağımsız şifreli mantıksal dump bulunur;
  tek zamanlanmış CI işi tek production yedeği sayılamaz.
- PITR, günlük veri kaybının elle yeniden girilemez hâle gelmesi, sözleşmeli RPO/RTO veya
  ürün sahibinin yazılı kritik-production kararıyla etkinleştirilir.
- PITR açıldığında tasarım hedefi sağlayıcının belgelediği sınırlar içinde **en fazla 2 dakika
  RPO**, ölçülecek **4 saat RTO**dur. OPS-002 gerçek veri boyutuyla geri yükleme tatbikatı yapar.
...
- Üretim maliyet onayı; seçilen compute, yedek, PITR varsa PITR, depolama/egress, staging ve
  mantıksal yedeği tek tahminde gösterir. PITR ve sürekli staging ayrı yükseltme tetikleridir;
  başlangıç bütçesine sessizce eklenemez.
...
- Migration aracı/yürütme modeli, modül migration'larından önce ayrı bir teknik karar olarak kesinleşir; repo/CI iskeleti yalnız onaylı aracın bağlantısını kurar. Secret mekanizması A-013 tarafından kesinleştirilir. A-009 yalnız monorepo/repo yapısı kararını verir.
...
- [x] Yerel, kapalı alfa, gerçek pilot ve kritik production maliyet profilleri ayrı gösterilmiştir.
- [x] Güncel sağlayıcı iddiaları resmî kaynaklarla doğrulanmıştır.

A-010 ortam sözleşmesi bu kademeli profile göre revize edilmiştir. OPS-001 ve OPS-002
tamamlanmadan gerçek kurum pilotunun yedekleme/geri yükleme yeterliliği kabul edilmiş sayılmaz.
