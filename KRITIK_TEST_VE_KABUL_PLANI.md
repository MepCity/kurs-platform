...
Amaç/risk: Askıya alınmış üyelik, eski sessionGeneration, geri alınmış TEACHER rolü veya kurum bağlamı taşımayan token kurum API'sine erişemez. Önkoşul ve fixture: T-A1 için ilk üç durum ayrı ayrı kurulur; ayrıca geçerli fakat `organization_membership_id` taşımayan global token hazırlanır. Bağlam: ORG-A tokenı ve kurum bağlamsız token. Adımlar: korunan okuma/yazma çağrılır; rol iptalinde atama ve devredilmiş izinler kontrol edilir; global tokenla kurum kapsamlı uç doğrudan çağrılır. HTTP/uygulama: geçersiz oturumda `401 UNAUTHENTICATED`/`SESSION_REVOKED`; kurum bağlamsız geçerli tokenla `403 ORGANIZATION_CONTEXT_REQUIRED`; hiçbir durum geçerli kurum oturumu sayılmaz. DB/transaction/audit/sync: rol iptali atomik kapanır; yeni iş/audit/sync oluşmaz. Yok: eski yetkiyle işlem, bağlamsız tokenla kurum verisi veya otomatik kuyruk gönderimi. Mobil/kuyruk: oturum geçersizse BLOCKED ve yeniden giriş; kurum bağlamı eksikse güvenli bağlam seçimi/değişimi gerekir. Otomasyon: API + transaction. Kanıt: dört token/üyelik sonucu, üyelik/atama/izin sorgusu. Kaynak: P-009 §4, §7.3, §9/2-3,14.
...
Amaç/risk: Sekiz devredilebilir izin yalnız kendi işlemini açar; hoca izin değiştirme ve özel öğrenci alanı tanımı yönetme mutlak sınırları aşılamaz, operasyonel veri tırmanışı oluşmaz. Önkoşul ve fixture: Veri güdümlü test, T-A1'e sırayla yalnız BRAND_MANAGE, MODULE_MANAGE, TEACHER_ACCOUNT_MANAGE, TEACHER_CLASS_ASSIGN, TEACHER_PERMISSION_VIEW, DEVICE_SESSION_REVOKE, CUSTOM_ATTENDANCE_STATUS_MANAGE ve GUARDIAN_CONTACT_VIEW verir; izin değiştirme ve özel alan tanımı yönetme ayrıca mutlak sınır olarak denenir; ADM-A kurum tavanı da sınanır. Bağlam: T-A1 ve ADM-A. Adımlar: sekiz izin kendi tekil işleminde denenir; hoca izin değiştirme ile `custom_field_definitions`/seçenek yönetimi her izin kombinasyonunda reddedilir. T-A1 kendi üzerinde atama yapmayı, kendi/başka hoca iznini değiştirmeyi ve kurum izniyle atanmadığı sınıf verisini ayrıca dener. HTTP/uygulama: yalnız açık tekil işlem başarı; tüm çapraz işlemler 403 veya kapsam gizliliğinde 404. DB/transaction/audit/sync: redlerde yetki/atama/tanım değişikliği yok; izinli yönetim işlemi denetlenir. Yok: izin türetme, kendi kapsamını genişletme, GUARDIAN_CONTACT_VIEW'un öğrenci görüntüleme/GUARDIAN_MANAGE sağlaması, özel alan şeması değiştirme, operasyonel veri sızıntısı veya ADM-A'nın kurum/rol tavanını aşması. Mobil/kuyruk: terminal red NEEDS_ATTENTION. Otomasyon: API + parametrik transaction testi. Kanıt: sekiz devredilebilir izin + iki mutlak sınır için işlem sonuç matrisi, atama ve sıfır yan-etki sorguları. Kaynak: P-003 §2.2 madde 5–12 ve 6b, §3.1, §4.1, §4.4, §6.2 karar 1–9; karar 7 için KAP-31.
...
| PLAN-004 — Özel öğrenci alanı tanımı yönetimi hoca için V1 mutlak sınırıdır | KAP-32 |
