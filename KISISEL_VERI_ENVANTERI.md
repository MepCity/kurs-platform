...
| 9 | Kimlik sağlayıcısı kullanıcı adı ve geçici parola | Global sağlayıcı hesabı / şifreli bir-kezlik teslim yüzeyi | Kullanıcı adı temel kişisel veri; parola yüksek riskli kimlik doğrulama sırrı | Parola ele geçirilirse hesabın tamamına erişim sağlar. Ham parola platform DB'sinin normal verisi değildir. [Karar] | Sağlayıcıya özgü create/reset/ilk parola değişimi A-004R1–A-004R3 ve IAM-001'de kesinleşir. Geçici parola gerekiyorsa A-013 anahtarıyla şifreli, en fazla 10 dakika geçerli ve tek-görüntülemelik teslim yüzeyinde tutulur; mobil/API loglarına yazılmaz. Teslim anında alıcının aynı kurumdaki aktif `ORG_ADMIN` rolü veya `TEACHER_ACCOUNT_MANAGE` izni yeniden doğrulanır. [Karar] | Parola özeti ve ham geçici parola normal platform tablosu/audit/idempotency sonucunda tutulmaz. Teslim süresi bitince veya tek okuma yanıtı kaybolsa dahi tekrar gösterilmez; yeni reset gerekir. Şifreleme anahtarı ve erişim sırrı A-013 tarafından yönetilir. [Karar] |
| 10 | OIDC issuer + subject eşlemesi | `user_identities` / global kullanıcı | Yüksek riskli kişisel veri (ürün içi sınıf; kalıcı çevrimiçi kimlik) | Sağlayıcı hesabını platform hesabına bağlar. [Karar] | Yalnız IAM kimlik sınırı ve yetkili global hesap işlemleri erişebilir; kullanıcı adı/e-posta ile otomatik eşleme yasaktır. [Karar] | Global kullanıcı arşivlenirken eşleme denetim izini koruyacak biçimde devre dışı bırakılır; hukuki imha süresi ayrı prosedürdedir. [Öneri/bekleyen karar] |
| 11 | Platform access/refresh tokenı ve uygulama kurulum cihaz kimliği | Kullanıcı oturumu (IAM) | Yüksek riskli kişisel veri (ürün içi sınıf) / teknik veri | Ele geçirilirse hesap oturumunun ele geçirilmesine yol açar. Cihaz kimliği donanım parmak izi değil, uygulamanın ürettiği rastgele UUID'dir. [Karar] | Erişim tokenı kısa ömürlü ve opaque; refresh token tek kullanımlı/aile-bazlı iptal edilebilir; yönetici kurum sınırında cihaz oturumlarını kapatabilir. [Karar] | Ham platform refresh/access tokenı DB/audit/logda tutulmaz; DB yalnız pepper'lı HMAC özetleri tutar. Sağlayıcı tokenının ilk değişim sonrası mobil yaşam döngüsü A-004R1–A-004R3/IAM-001'de kesinleşir; kalıcı uygulama oturumu platform tokenlarıyla sürer. [Karar] |
