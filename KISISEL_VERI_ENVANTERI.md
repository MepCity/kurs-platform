...
| 9 | Keycloak kullanıcı adı ve geçici parola | Keycloak global hesabı / şifreli bir-kezlik teslim yüzeyi | Kullanıcı adı temel kişisel veri; parola yüksek riskli kimlik doğrulama sırrı | Parola ele geçirilirse hesabın tamamına erişim sağlar. Ham parola platform DB'sinin normal verisi değildir. [Karar] | Worker geçici parolayı üretir; yalnız Keycloak create/reset/`UPDATE_PASSWORD` akışında işler. Create doğrulanınca A-013 anahtarıyla şifreli, oluşturulduğu andan itibaren en fazla 10 dakika geçerli ve tek-görüntülemelik teslim yüzeyinde tutulur; mobil/API loglarına yazılmaz. Teslim anında alıcının aynı kurumdaki aktif `ORG_ADMIN` rolü veya `TEACHER_ACCOUNT_MANAGE` izni yeniden doğrulanır. [Karar] | Parola özeti, ham geçici parola ve `UPDATE_PASSWORD` durumu normal platform tablosu/audit/idempotency sonucunda tutulmaz. Teslim süresi bitince veya tek okuma yanıtı kaybolsa dahi tekrar gösterilmez; yeni reset gerekir. Şifreleme anahtarı ve erişim sırrı A-013 tarafından yönetilir. [Karar] |
