...
class RuntimeEnvironmentConfigurationTests {

	private final ApplicationContextRunner contextRunner = new ApplicationContextRunner()
			.withUserConfiguration(RuntimeEnvironmentConfiguration.class);

	@Test
	void contextFailsFastWhenRequiredEnvironmentValuesAreMissing() {
		contextRunner.run(context -> {
			assertThat(context).hasFailed();
			assertThat(context.getStartupFailure()).hasRootCauseMessage("KURS_PLATFORM_ENVIRONMENT is required");
		});
	}

	@Test
	void contextLoadsWhenSyntheticValuesAreProvidedByTestInput() {
		contextRunner
				.withPropertyValues(
				...
				.run(context -> {
					...
					assertThat(context).hasSingleBean(RuntimeEnvironmentSettings.class);
				});
	}
}
