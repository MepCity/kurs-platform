...
CREATE POLICY platform_administrators_select_device_support ON platform_administrators FOR SELECT TO iam_runtime USING (
 ...
 AND user_id=NULLIF(current_setting('app.iam_actor_user_id',true),'')::uuid AND revoked_at IS NULL
);
...
CREATE POLICY organization_memberships_select_device_revoke ON organization_memberships FOR SELECT TO iam_runtime USING (
 current_user='iam_runtime' AND current_setting('app.iam_operation_scope',true)='ORGANIZATION'
 AND current_setting('app.iam_operation_code',true)='DEVICE_SESSION_REVOKE'
 AND organization_id=NULLIF(current_setting('app.iam_target_organization_id',true),'')::uuid
 AND (iam_device_session_revoke_authorized(NULLIF(current_setting('app.iam_actor_user_id',true),'')::uuid, organization_id)
      OR (current_setting('app.iam_platform_admin_support_access',true)='true' AND EXISTS (SELECT 1 FROM platform_administrators pa
          WHERE pa.user_id=NULLIF(current_setting('app.iam_actor_user_id',true),'')::uuid AND pa.revoked_at IS NULL)))
);
CREATE POLICY organization_memberships_update_device_revoke ON organization_memberships FOR UPDATE TO iam_runtime USING (
 current_user='iam_runtime' AND current_setting('app.iam_operation_scope',true)='ORGANIZATION'
 AND current_setting('app.iam_operation_code',true)='DEVICE_SESSION_REVOKE'
 AND organization_id=NULLIF(current_setting('app.iam_target_organization_id',true),'')::uuid
 AND id=NULLIF(current_setting('app.iam_target_membership_id',true),'')::uuid
 AND (iam_device_session_revoke_authorized(NULLIF(current_setting('app.iam_actor_user_id',true),'')::uuid, organization_id)
      OR (current_setting('app.iam_platform_admin_support_access',true)='true' AND EXISTS (SELECT 1 FROM platform_administrators pa
          WHERE pa.user_id=NULLIF(current_setting('app.iam_actor_user_id',true),'')::uuid AND pa.revoked_at IS NULL)))
...
CREATE POLICY organization_membership_roles_select_device_revoke ON organization_membership_roles FOR SELECT TO iam_runtime USING (
 ...
 AND current_setting('app.iam_operation_code',true)='DEVICE_SESSION_REVOKE'
 AND organization_id=NULLIF(current_setting('app.iam_target_organization_id',true),'')::uuid
 AND organization_membership_id IN (NULLIF(current_setting('app.iam_target_membership_id',true),'')::uuid)
 AND (iam_device_session_revoke_authorized(NULLIF(current_setting('app.iam_actor_user_id',true),'')::uuid, organization_id)
      OR (current_setting('app.iam_platform_admin_support_access',true)='true' AND EXISTS (SELECT 1 FROM platform_administrators pa
          WHERE pa.user_id=NULLIF(current_setting('app.iam_actor_user_id',true),'')::uuid AND pa.revoked_at IS NULL)))
);
...
CREATE POLICY refresh_token_families_select_device_revoke ON refresh_token_families FOR SELECT TO iam_runtime USING (
 current_user='iam_runtime' AND current_setting('app.iam_operation_scope',true)='ORGANIZATION'
 AND current_setting('app.iam_operation_code',true)='DEVICE_SESSION_REVOKE'
 AND organization_membership_id=NULLIF(current_setting('app.iam_target_membership_id',true),'')::uuid
);
CREATE POLICY refresh_token_families_update_device_revoke ON refresh_token_families FOR UPDATE TO iam_runtime USING (
 current_user='iam_runtime' AND current_setting('app.iam_operation_scope',true)='ORGANIZATION'
 AND current_setting('app.iam_operation_code',true)='DEVICE_SESSION_REVOKE'
 AND organization_membership_id=NULLIF(current_setting('app.iam_target_membership_id',true),'')::uuid AND revoked_at IS NULL
...
CREATE POLICY refresh_token_families_select_device_self_revoke ON refresh_token_families FOR SELECT TO iam_runtime USING (
 ...
 AND user_id=NULLIF(current_setting('app.iam_actor_user_id',true),'')::uuid
 AND trusted_device_id=NULLIF(current_setting('app.iam_target_device_id',true),'')::uuid
);
CREATE POLICY refresh_token_families_update_device_self_revoke ON refresh_token_families FOR UPDATE TO iam_runtime USING (
 current_user='iam_runtime' AND current_setting('app.iam_operation_scope',true)='IAM_AUTH'
 AND current_setting('app.iam_operation_code',true)='DEVICE_SELF_REVOKE'
 AND user_id=NULLIF(current_setting('app.iam_actor_user_id',true),'')::uuid
 AND trusted_device_id=NULLIF(current_setting('app.iam_target_device_id',true),'')::uuid AND revoked_at IS NULL
) WITH CHECK (revoked_at=transaction_timestamp());
...
CREATE POLICY refresh_tokens_select_device_revoke ON refresh_tokens FOR SELECT TO iam_runtime USING (
 current_user='iam_runtime' AND current_setting('app.iam_operation_scope',true)='ORGANIZATION'
 AND current_setting('app.iam_operation_code',true)='DEVICE_SESSION_REVOKE'
 AND EXISTS (SELECT 1 FROM refresh_token_families f WHERE f.id=family_id AND f.organization_membership_id=NULLIF(current_setting('app.iam_target_membership_id',true),'')::uuid)
);
...
CREATE POLICY refresh_tokens_select_device_self_revoke ON refresh_tokens FOR SELECT TO iam_runtime USING (
 current_user='iam_runtime' AND current_setting('app.iam_operation_scope',true)='IAM_AUTH'
 AND current_setting('app.iam_operation_code',true)='DEVICE_SELF_REVOKE'
 AND EXISTS (SELECT 1 FROM refresh_token_families f WHERE f.id=family_id AND f.user_id=NULLIF(current_setting('app.iam_actor_user_id',true),'')::uuid
            AND f.trusted_device_id=NULLIF(current_setting('app.iam_target_device_id',true),'')::uuid)
);
...
CREATE POLICY audit_logs_insert_iam_device_revoke ON audit_logs FOR INSERT TO iam_runtime WITH CHECK (
 ...
 AND actor_user_id=NULLIF(current_setting('app.iam_actor_user_id',true),'')::uuid AND requires_target_entity
 ...
 AND ((action_type='DEVICE_SELF_REVOKED' AND event_scope='GLOBAL' AND organization_id IS NULL AND target_entity_id=NULLIF(current_setting('app.iam_actor_user_id',true),'')::uuid
  ...
  OR (action_type='PLATFORM_DEVICE_REVOKED' AND event_scope='GLOBAL' AND organization_id IS NULL
       AND target_entity_id=NULLIF(current_setting('app.iam_target_user_id',true),'')::uuid
       AND (event_metadata->>'trustedDeviceId')=current_setting('app.iam_target_device_id',true)
       AND EXISTS (SELECT 1 FROM trusted_devices d WHERE d.id=NULLIF(current_setting('app.iam_target_device_id',true),'')::uuid
                   AND d.user_id=NULLIF(current_setting('app.iam_target_user_id',true),'')::uuid)
  ...
  OR (action_type='DEVICE_SESSION_REVOKED' AND event_scope='ORGANIZATION' AND target_entity_type='USER'
       ...
       AND EXISTS (SELECT 1 FROM organization_memberships m WHERE m.id=NULLIF(current_setting('app.iam_target_membership_id',true),'')::uuid
  ...
  OR (action_type='PLATFORM_ADMIN_ORG_ACCESS' AND event_scope='ORGANIZATION' AND target_entity_type='ORGANIZATION'
       AND target_entity_id=organization_id AND organization_id=NULLIF(current_setting('app.iam_target_organization_id',true),'')::uuid
       AND current_setting('app.iam_operation_scope',true)='ORGANIZATION' AND current_setting('app.iam_operation_code',true)='DEVICE_SESSION_REVOKE'
       AND current_setting('app.iam_platform_admin_support_access',true)='true'
       AND EXISTS (SELECT 1 FROM platform_administrators pa WHERE pa.user_id=NULLIF(current_setting('app.iam_actor_user_id',true),'')::uuid AND pa.revoked_at IS NULL)))
);
