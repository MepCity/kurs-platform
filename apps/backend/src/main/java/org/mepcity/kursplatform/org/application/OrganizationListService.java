...
public final class OrganizationListService {
    ...
    public Result list(String token, Query query, String requestId) {
        ...
        try {
            credential = sessions.resolveCredential(token);
        ...
        if (!actor.isGlobalPlatformAdmin() && query.supplied()) {
    ...
    private Result global(ActiveSession actor, Query query, String requestId) {
        OrganizationListCursor.Context context =
                query.context(actor.userId(), OrganizationListTransaction.Scope.GLOBAL);
        ...
        return transactions.execute(actor.userId(), OrganizationListTransaction.Scope.GLOBAL, null, () -> {
            ...
            for (Organization item : items) {
                audits.write(
                        new AuditEvent.Factory(requestId)
                                .platformAdminOrgAccess(
                                        UUID.randomUUID(),
                                        item.id(),
                                        actor.userId(),
                                        item.id(),
    ...
    public record Query(
        ...
        OrganizationListCursor.Context context(UUID actor, OrganizationListTransaction.Scope scope) {
            return new OrganizationListCursor.Context(actor, scope, status, search, sort, order, limit);
        }

        OrganizationListQuery repositoryQuery(OrganizationListCursor cursor, int size) {
            OrganizationListQuery.Position position =
                    cursor == null
                            ? null
                            : sort == OrganizationListQuery.Sort.NAME
                                    ? new OrganizationListQuery.NamePosition(
                                            ((OrganizationListCursor.Name) cursor.lastValue()).value(),
                                            cursor.lastOrganizationId())
                                    : new OrganizationListQuery.CreatedAtPosition(
                                            ((OrganizationListCursor.CreatedAt) cursor.lastValue()).value(),
                                            cursor.lastOrganizationId());
            return new OrganizationListQuery(status, search, sort, order, size, position);
        }

        OrganizationListCursor.LastValue lastValue(Organization organization) {
            return sort == OrganizationListQuery.Sort.NAME
                    ? new OrganizationListCursor.Name(organization.name())
                    : new OrganizationListCursor.CreatedAt(organization.createdAt());
        }
    }
